...
- **Proof parsing (`snapshot-parse.ts`):** `SnapshotProof.solver` is now **optional** — `parseProof` requires only `goal` (a record in the index ⇒ proved) and treats an absent / `∅` solver as undefined. The hardest proofs are often older archived ones whose attribution is **inferred from git history** (no explicit `solver≜`); requiring a solver dropped them — including the lone difficulty-5 (`realization-determines-counts`), a genuine mathlib-absent result that failed two ~31-min high-effort attempts.
- **Derivations (`derive.ts`):** `deriveGoalSolverMap` stays **active-only AND explicit-solver-only** (proof graph / boards unchanged — solverless proofs are skipped there). New `deriveShowcaseSolverMap` (active + archived, **including solverless** proofs with `solver: ''`, active wins) and `deriveGoalMetaMap` (goal → `{difficulty, status}`). The Showcase / proof-detail render solverless proofs as "attribution inferred".
